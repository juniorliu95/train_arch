...
class HeatMap(object):
    ...
    def heatmap(self, save_as=None, base=None, data=None, r=1):   
        if not data.any():
            data = self.data  # disposed gray map, 2-d
...
def main():
    ...
    data_cam = np.load(TOP_DIR+PRED_DIR+"cam.npy")
    # data_true_label_df = pd.read_csv(TOP_DIR+PRED_DIR+"ground_true_label.csv")
    ...
    for index, img_filename in enumerate(image_indexes):       
        if not index%100:         
            ...
            if duration:
                avgtime = sum(duration) / len(duration)
                duration = []
                print('Average time create a heatmap: %d sec' % avgtime)
        img_addr = read_process_image(TOP_DIR+IMG_DIR+img_filename)
        mask_addr = TOP_DIR + MASK_DIR + img_filename
        hm = HeatMap(base=img_addr, mask=mask_addr)
        for class_id in range(14):
            pred_label = data_pred_label_df.loc[index, label_dict[class_id]]
            # ture_label = data_true_label_df.loc[data_true_label_df["filename"]==img_filename, label_dict[class_id]].values[0]
            if label_dict[class_id] != IMG_DIR[:-1]:
                continue
            if pred_label:
...
def drawcounter():
    img = cv2.imread("/home/huiying/sdb1/HY_chestfilm/test_personal/pathology_from_nih/heatmap/Atelectasis/11.png")
    img_counter = cv2.imread("/home/huiying/sdb1/HY_chestfilm/test_personal/pathology_from_nih/ateletasis_seg_mask/00000080_001.png", 0)
    img_resize = cv2.resize(img_counter,(_IMG_WIDTH, _IMG_HEIGHT),interpolation = cv2.INTER_AREA)
    _, contours, _ = cv2.findContours(img_resize, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    cv2.drawContours(img, contours, contourIdx=0, color=(255,0,0), thickness=2)
    cv2.imwrite("/home/huiying/sdb1/HY_chestfilm/test_personal/pathology_from_nih/heatmap/Atelectasis/12.png", img)
